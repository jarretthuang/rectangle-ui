...
const TEXTAREA_TEXT = "text-sm font-semibold text-primary-900 placeholder:text-primary-700/70 dark:text-primary-100 dark:placeholder:text-primary-300/70";
...
@Component({
  ...
  imports: [NgClass],
  template: `
    <textarea [placeholder]="placeholder" [disabled]="disabled" [value]="value()" [ngClass]="styleClasses" (input)="onInput($event)"></textarea>
  `,
...
export class TextareaComponent {
  ...
  value = model<string>("");
  protected readonly styleClasses: string[] = [TEXTAREA_BACKGROUND, TEXTAREA_TEXT, TEXTAREA_LAYOUT, TEXTAREA_ANIMATION];
  onInput(event: Event) { this.value.set((event.target as HTMLTextAreaElement).value); }
}
